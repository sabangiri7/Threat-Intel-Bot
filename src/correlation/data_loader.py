...
def setup_folder_structure():
    ...
    logging.info("Creating __init__.py files...")
    init_files = [
...
def save_enriched_iocs(iocs: list, output_path: str = "data/enriched_iocs.json"):
    ...
    logging.info(f"Saving {len(iocs)} IOCs...")
    output = Path(output_path)
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(json.dumps(iocs, indent=2))
    logging.info(f"Saved to: {output} ({output.stat().st_size / 1024:.1f} KB)")


def print_summary(iocs: list):
    """Print summary of generated IOCs."""
    logging.info("IOC Type Breakdown:")
    types = {}
    for ioc in iocs:
        t = ioc["ioc_type"]
